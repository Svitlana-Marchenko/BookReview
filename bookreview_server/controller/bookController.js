...
const getBooks = async (req, res) => {
    try {
        ...
        let books
        if (limit > 0) {
            books = await bookService.findBooksPagination(query, page, limit);
        } else {
            books = await bookService.findAllBooks(query);
        }
...
const postBook = async (req, res) => {
    const { title, authorId, genre, year, page } = req.body;
    ...
    const data = { title, author, genre, year: yearOfPublished, page: numberOfPage };
    try {
